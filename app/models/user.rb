class User < ActiveRecord::Base
  ...
  scope :with_active_token, ->{where "expiration > ?", Time.now.to_i  }
  ...
  def self.find_for_vkontakte_oauth access_token
    ...
    if user = User.where(:id => access_token.uid).first
      user.update_attributes(session_token:access_token.credentials.token,last_sign_in_at:Time.now,expiration:access_token.credentials.expires_at)
      user
